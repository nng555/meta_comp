...
from utils import mask_matrix, reshape_x_and_lengths, MySequential, MyLinear
...
class XISAB(nn.Module):
    ...
    def forward(self, X, Y, xlengths, ylengths):
        ...
        HX = self.mabx(HX, HY, lengths=xlengths)
        HY = self.mabx(HY, HX, lengths=ylengths)
...
class SetTransformer2(nn.Module):
    def __init__(self, n_inputs=2, n_outputs=1, n_enc_layers=2,
        ...
        for i in range(n_enc_layers):
            layers.append(XISAB(dim_hidden, dim_hidden, num_heads, num_inds, norm=norm, sample_size=sample_size))
        #if norm != "none":
        ...
        self.dec = MySequential(
            SAB(dim_hidden, dim_hidden, num_heads, norm="none", sample_size=2),
            SAB(dim_hidden, dim_hidden, num_heads, norm="none", sample_size=2),
